...
Component({
    ...
    methods: {
        ...
        RPG_TWO_POINT(VAL, value) {
            ...
            if (str != -1 && VAL.length - str == 3 && lastWard !== '+' && lastWard !== '-') {
                NUM_STR = isNaN(value)
            }
            return NUM_STR
        },
        ...
        TYPE_DETECTION(VALUE, TYPE, TARGET) {
            ...
            switch (TYPE) {
                case 'num':
                    SHOW = (SHOW == 0 && VALUE != '.' && SHOW.length == 1) ? VALUE : SHOW + VALUE
                    HIDE += VALUE
                    break;
                case 'finish':
                    SHOW = SHOW
        ...
        CACULATE_THE_SUM(ENTERNUM, SHOW, HIDE) {
            HIDE = String(HIDE)
            SHOW = String(SHOW)
            if (ENTERNUM !== 'add' && ENTERNUM !== 'reduce' && ENTERNUM !== 'finish') return { SHOW, HIDE }
        ...
        async setBill() {
            ...
            let bill_Data = {}
            let data = this.data
            bill_Data.price = data.show_num //价格
            bill_Data.remark = data.remark
            bill_Data.classid = this.id
            bill_Data.date = data.date
            bill_Data.date_unix = DAY_UNIX(data.date)
            bill_Data.create_date = DAY_UNIX(new Date()) //时间戳

            console.log('--------', data.date, bill_Data.date_unix);
